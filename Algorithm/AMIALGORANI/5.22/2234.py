...
graph = []
new_graph = [[0] * n for i in range(m)]
for i in range(m):
    graph.append(list(map(int, input().split())))
for i in range(m):
    for j in range(n):
        graph[i][j] = bin(graph[i][j])[2:].zfill(4)
...
def bfs(start_x, start_y):
    ...
    while new_q:
        x, y = new_q.popleft()
        new_graph[x][y] = room_space
    return room_space


first = 0  # 방의 개수
second = 0  # 가장 넓은 방의 넓이
third = 0  # 하나의 벽을 제거하여 얻을 수 있는 가장 넓은 방의 크기
for i in range(m):
    for j in range(n):
        if not visited[i][j]:
            temp = bfs(i, j)
