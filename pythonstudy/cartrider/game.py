...
class Player:
    ...
    def add_play_record(self, record_in_hr):
        ...
        temp = record_in_hr*60*60
        self.play_records.append(temp)
...
class Game:
    ...
    def set_players(self):
        ...
        print("*******************게임 접속*******************")
        for i in range(5):
            name=input(f"Player {i+1} 의 이름을 입력해주세요:")
            player=Player(name)
    ...
    def start_game(self):
        ...
        for i in range(len(self.player_list)):
            character_number=int(input("{}의 캐릭터 선택 차례입니다. 1,2,3 중 하나의 값을 입력해주세요. ".format(self.player_list[i].name)))
            print("{}의 고유 속도는 시속 {}입니다.".format(self.player_list[i].name,self.character_list[character_number-1].speed))
            self.player_list[i].speed=self.character_list[character_number-1].speed
        print()
        # TODO (3): 두 종류의 아이템을 생성해 아이템 목록(self.item_list)에 추가해주세요.
        item1=Item("banana_slip",-random.randint(20,40))
        item2=Item("booster",random.randint(30,60))
        self.item_list.append(item1)
        self.item_list.append(item2)
        #for i in range(2):
        #    print(self.item_list[i].item_type,self.item_list[i].speed_change)
    def play_round(self,round):
        ...
        track_length=random.randint(5,30)
        print(f"이번 라운드의 트랙 길이는 {track_length} km입니다.")
        # TODO (1) - 2: 5명의 플레이어에게 아이템을 랜덤하게 적용시키고, 적용된 아이템과 적용 후 플레이어의 속도를 출력해주세요.
        print("[아이템 적용]")
        for i in range(5):
            itemnum=random.randint(0,1)
            #print(itemnum)
            player_round_speed = self.player_list[i].speed + self.item_list[itemnum].speed_change
            if itemnum==0:
                fighting="ㅠㅠ"
            elif itemnum==1:
                fighting="~~"
            print(f"Player {self.player_list[i].name}는 {self.item_list[itemnum].item_type}덕분에 시속 {player_round_speed}로 이번 트랙을 질주합니다! 화이팅{fighting}")
            record = track_length/player_round_speed
        ...
        print("[라운드 결과]")
        for i in range(5):
            print(f"Player {self.player_list[i].name}의 주행 시간: {self.player_list[i].play_records[self.round]}")
    def game_result(self):
        ...
        for i in range(5):
            temp=0
            for play_record in self.player_list[i].play_records:
                temp+=play_record
            result[self.player_list[i].name]=temp
        #print(result)
        new_result=sorted(result.items(),key=lambda x: x[1])
        #print(new_result)
        for i in range(3):
           print(f"{i+1} 위: {new_result[i][0]}(총 주행시간:{new_result[i][1]}초")
    def game(self):
        """
        - 게임 운영을 위한 함수입니다.
        - 별도의 코드 작성이 필요 없습니다.
